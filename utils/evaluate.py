...
def test_one_user(x):
    # rating: user u's ratings for user u; u: uid
    rating, u, train_user_set, test_user_set, n_items, Ks, test_flag = x
...
def test(model, user_dict, n_params, gamma):
    ...
    n_user_batchs = math.ceil(n_test_users / u_batch_size)

    count = 0

    entity_gcn_emb, user_gcn_emb = model.generate(gamma)

    with multiprocessing.get_context("spawn").Pool(cores) as pool:
        for u_batch_id in tqdm(range(n_user_batchs), desc="Evaluating"):
            ...
            if batch_test_flag:
                # batch-item test
                n_item_batchs = math.ceil(n_items / i_batch_size)
            ...
            user_batch_rating_uid = [
                (rate_batch[i], user_list_batch[i], train_user_set, test_user_set, n_items, Ks, args.test_flag)
